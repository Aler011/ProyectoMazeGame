...
public class GameGui extends JFrame implements ActionListener {
    ...
    public GameGui() {
        ...
        tk = new TimeKeeper();
        pack();
        setVisible(true);//show our menu bar and shagLabel
    }
    ...
    private class MyKeyHandler extends KeyAdapter {
        ...
        public void keyPressed(KeyEvent theEvent) {
            switch (theEvent.getKeyCode()) {
                case KeyEvent.VK_UP: {
                    theArc.playerMove(-1, 0, scrapMatrix, fl.dimondCount());//Captura un movimiento del teclado hacia arriba
                    loadMatrixGui("updateLoad");//reload the gui to show the move
                    if (theArc.getLevel() == true) {
                        nextLevelLoad();//Se carga al siguiente nivel si el jugador toca la puerta al moverse.
                    }
                    break;
                }
                case KeyEvent.VK_DOWN: {
                    theArc.playerMove(1, 0, scrapMatrix, fl.dimondCount());//Captura un movimiento del teclado hacia abajo
                ...
                case KeyEvent.VK_LEFT: {
                    theArc.playerMove(0, -1, scrapMatrix, fl.dimondCount());//Captura un movimiento del teclado hacia la izquierda
                    loadMatrixGui("updateLoad");//see above
                    if (theArc.getLevel() == true) {
                        nextLevelLoad();//see above
                    }
                ...
                case KeyEvent.VK_RIGHT: {
                    theArc.playerMove(0, 1, scrapMatrix, fl.dimondCount()); //Captura un movimiento del teclado hacia la derecha
                    loadMatrixGui("updateLoad");//see above
                    if (theArc.getLevel() == true) {
                        nextLevelLoad();//see above
                    }
    ...
    //Método para realizar acciones(eventos) en el juego maze
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals("Exit")) {  //Se detiene y sale del juego si se selecciona esta opcción.
            new Timer(1000, updateCursorAction).stop();
            System.exit(0); //exit the system.   
        } else if (e.getActionCommand().equals("New Game")) { //Se crea un nuevo juego si se selecciona esta opcción.
            return; //maybe implent this feature later
        } else if (e.getActionCommand().equals("EnterName")) {//El jugador ingresa su nombre si se selecciona esta opcción.
        ...
        } else if (e.getActionCommand().equals("Open")) { //Se abre un archivo del juego maze para poder iniciar el juego.
            JFileChooser chooser = new JFileChooser();
            int returnVal = chooser.showOpenDialog(this);
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                fl.loadFile(chooser.getSelectedFile().getName());//load the file
    ...
    public void loadMatrixGui(String event) {
        if (event == "newLoad") {
            ...
            timeCalc.calcTimeforMaze(fl.dimondCount(), fl.getMatrixSizeRow(), fl.getMatrixSizeColumn()); //Se le pasan los parametros recolectados mas arriba del código para poder calcular el tiempo
            ...
            newPanel = new JPanel();
            newPanel.setLayout(new GridLayout(fl.getMatrixSizeRow(), fl.getMatrixSizeColumn())); //set the panel for the game to the size of the matrix      
        ...
        for (int i = 0; i < labelMatrix.length; i++) {
            for (int j = 0; j < labelMatrix[i].length; j++) {
                labelMatrix[i][j] = mo = new mazeObject(scrapMatrix[i][j]); //add the maze images into the gui
            }
        }
        cp.add(newPanel);
